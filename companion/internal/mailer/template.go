...
// appDecisionTmplSrc is the HTML source for owner approval/rejection emails.
const appDecisionTmplSrc = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#0a0e1a;padding:32px 16px;">
  <tr><td align="center">
    <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#0d1526;border:1px solid #1e3a5f;border-radius:8px;overflow:hidden;">
        ...
        <td style="padding:24px 28px 20px;border-bottom:1px solid #1e3a5f;">
          <table cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td style="vertical-align:middle;">
                <span style="font-size:17px;font-weight:700;color:#e2e8f0;letter-spacing:0.4px;">SCID</span>
                <span style="font-size:13px;color:#64748b;margin-left:6px;">Account Update</span>
              </td>
            </tr>
          </table>
        </td>
      ...
      <tr>
        <td style="padding:28px 28px 0;">
          <p style="margin:0 0 6px;font-size:18px;font-weight:600;color:#e2e8f0;">{{.OutcomeTitle}}</p>
          <p style="margin:0 0 24px;font-size:14px;color:#94a3b8;">{{.OutcomeMessage}}</p>
            ...
            <tr>
              <td style="padding:18px 20px;">
                <table width="100%" cellpadding="0" cellspacing="0">
                  ...
                  {{template "row" list "App ID" .AppID}}
                  {{template "row" list "Submitted by" .OwnerHandle}}
                  {{template "row" list "Status" .OutcomeTitle}}
                  {{if .Reason}}{{template "row" list "Reason" .Reason}}{{end}}
                </table>
              </td>
            </tr>
          </table>
          ...
          <table cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
            <tr>
              <td style="background:#00d4ff;border-radius:6px;">
                <a href="{{.ActionURL}}" style="display:inline-block;padding:11px 24px;font-size:14px;font-weight:600;color:#0a0e1a;text-decoration:none;letter-spacing:0.2px;">{{if .ActionLabel}}{{.ActionLabel}}{{else}}Open your application{{end}} &#8594;</a>
              </td>
            </tr>
          </table>
          {{end}}
